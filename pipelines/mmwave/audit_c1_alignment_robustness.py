...
def main():
    ...
    manifest = {
        ...
        "raw_adc_read": False, "front_end_changed": False,
        "ecg_reference_role": "diagnostic lag optimization reference for ECG/radar beat matching",
        "ecg_used_for_diagnostic_lag_optimization": True,
        "ecg_used_for_formal_tuning": False,
        "status_scope": "DIAGNOSTIC_ONLY; not a formal HRV result",
    ...
    lines = [
        ...
        f"Status: `{status}`",
        "",
        "本审计只读取 C1c replay 与 C1d backend 已保存的 ECG/radar beat timestamps；ECG 被用作诊断性 lag optimization 的匹配参考，但不是正式 HRV 调参或性能验证终点。没有读取 raw ADC，没有修改前端、VMD 或峰检测。delay 符号与冻结 evaluator 一致：adjusted radar = radar - delay_ms/1000。",
        "",
        "## Primary ±75 ms",
        "",
    ...
    lines += [
        ...
        "",
        "- Lag sweep is `DIAGNOSTIC_ONLY`; ECG is the diagnostic lag-optimization reference, not a formal HRV result or tuning claim.",
