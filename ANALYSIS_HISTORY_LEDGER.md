...
### 2026-09-11：Issue #33 冻结队列修复与逐 probe 对照 — PARTIAL

复用理由：旧 adapter 的窗口右端、有效起点、内部心率汇总和来源写入不符合现有合同；producer 已有对应实现，未设计新算法。基于 main 9cbaca004aa1f6e4977b620e4efda247f343f932，固定 J72/E44、2320 行，冻结旧哈希后隔离运行。385 个端点命中行在联合修复后有变化，实际 Block 截断 0；观察状态 2200→2180，新增 20 行帧数不匹配。完整表和逐行差异仅本地保存，Git-safe 汇总与可复用入口见 `docs/results/2026-09-11_ISSUE33_PROBE_REPAIR/REPORT.md`。内部可用率仍全 1.0，不因修复自动成为有效预测变量。质量审计完成，未训练模型；历史 E 精确运行来源 UNKNOWN，生理正式资格仍未通过。只有输入/合同改变或复现需求才重跑，禁止为了取得不同数值重复当前计算。
