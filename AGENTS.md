...
`docs/00_治理/项目级进度与时空状态地图_20260829.md` 是当前阶段、已完成、进行中、阻塞、已被替代、冻结结论和下一决策的统一入口。新建议或重大返工必须先核对该地图，并通过中央治理的 Reuse Gate 与 Stage Gate；局部 bug、单一指标或新细节不能单独重开已完成/冻结工作。重开时必须保留旧结论、写明新证据和验收标准。
