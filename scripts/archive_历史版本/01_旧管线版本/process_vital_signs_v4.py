"""
Vital-sign extraction v4.

This version focuses on respiration enhancement only. Heart extraction stays on
the stable baseline path, while respiration can optionally use VMD on the
breath-bin displacement.
"""
...
def _breath_mode_score(mode_signal, freqs, br_mask, br_freq_hint):
    _, pxx = signal.periodogram(mode_signal, fs=FS, window="hann")
    band_energy = float(np.sum(pxx[br_mask]))
    if band_energy <= 0:
        return -np.inf, None

    dom_idx = np.argmax(pxx[br_mask])
    dom_freq = float(freqs[br_mask][dom_idx])
    if not 0.1 <= dom_freq <= 0.5:
        return -np.inf, dom_freq

    score = band_energy
    if br_freq_hint is not None:
        score /= (1.0 + abs(dom_freq - br_freq_hint))
    return score, dom_freq


def separate_vmd_breath_only(disp_br, br_freq_hint=None):
    """
    Use VMD only on the breath-bin displacement.
    ...
    u, _, omega = VMD(disp_br, alpha=1000, tau=0, K=4, DC=False, init=1, tol=1e-6)

    freqs = np.fft.rfftfreq(len(disp_br), d=1 / FS)
    ...
    for k in range(u.shape[0]):
        score, dom_freq = _breath_mode_score(u[k], freqs, br_mask, br_freq_hint)
    ...
    return breath, {
        "method": "vmd_breath_only",
        "backend": backend,
        "breath_mode": int(best_idx),
...
def analyze(data_path, method="bp", output_dir=None):
    ...
    heartbeat = _sos_bandpass(disp_hr, 0.8, 2.5)
    ...
    if method == "vmd_breath":
        breath, sep_info = separate_vmd_breath_only(disp_br, br_freq_hint=br_freq)
    else:
        breath = breath_bp
        sep_info = {
            "method": "bp_dual_bin",
    ...
    hp = detect_peaks_heart(heartbeat)
...
def main():
    parser = argparse.ArgumentParser(description="Vital-sign extraction v4")
    parser.add_argument("data_path", help="Path to datacube.bin or npz")
    parser.add_argument(
        "--method",
        "-m",
        choices=["bp", "vmd", "vmd_breath"],
        default="bp",
        help="bp=baseline; vmd_breath=VMD for breath only",
    )
