...
def load_behavior_span(subject_dir: Path) -> tuple[int | None, int | None]:
...
def _heartband_assess(phi_det: np.ndarray, hr_hint: float | None):
    ...
    phi_clean = _notch_harmonics(phi_det, br_freq, n_harm=HARM_N_MAX)
    ...
    # 谐波频点排除: 快呼吸 4-5 次谐波落入心跳带, 主频检测时剔除
    pxx = _harmonic_exclude(pxx, f, br_freq)
    p_hb = pxx[mask_hb]
...
def assess_window(iq: np.ndarray, hr_hint: float | None = None,
                  use_spc: bool = False, max_bin: int = MAX_TARGET_BIN) -> dict:
    ...
    zone = slice(MIN_TARGET_BIN, max_bin + 1)
...
def _harmonic_exclude(pxx: np.ndarray, freqs: np.ndarray, br_freq: float | None,
    ...
    if br_freq is None:
        return pxx
    pxx = pxx.copy()
    for k in range(1, HARM_N_MAX + 1):
        fk = br_freq * k
        if fk < freqs[0] or fk > freqs[-1]:
            continue
        mask = (freqs >= fk - width) & (freqs <= fk + width)
        if np.any(mask):
            pxx[mask] = np.median(pxx)
    return pxx
...
def main():
    ...
    span_start, span_end = load_behavior_span(mm_dir.parent)
    if span_start is None:
        ...
        print(f"  [行为轴截断] 分析范围: "
              f"{(t_start_ms - py_ms[0]) / 1000:.0f}s → {(t_end_ms - py_ms[0]) / 1000:.0f}s "
              f"(全数据 0 → {(py_ms[-1] - py_ms[0]) / 1000:.0f}s)")
    ...
    if hr_range is not None:
        for r in rows:
            if r.get("ok") and r.get("hr_bpm"):
                if not (hr_range[0] <= r["hr_bpm"] <= hr_range[1]):
                    r["ok"] = False
                    r["reason"] = "hr_out_of_range"
                    n_hr_out += 1
        print(f"  [2b] 生理 HR 范围过滤 ({hr_range[0]:.0f}-{hr_range[1]:.0f} bpm): "
              f"剔除 {n_hr_out} 窗")
