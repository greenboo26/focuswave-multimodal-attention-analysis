...
数据: 09_PREEXP-SUB{XXX}-FULL/sub{XXX}_full_windows.json（analyze_mmwave_full 产出）
输出: output/09_PREEXP-SUBJECTS-COMPARE/
...
OUT_DIR = OUT_ROOT / "09_PREEXP-SUBJECTS-COMPARE"
...
def load_ok_windows(subject: str) -> tuple[list[dict], list[dict]]:
    ...
    返回:
        (ok_windows, probes): 可信窗列表（quality=ok, 含生理字段）,
                              探针列表
    """
    path = OUT_ROOT / f"09_PREEXP-SUB{subject}-FULL" / f"sub{subject}_full_windows.json"
    if not path.exists():
        return [], []
...
def main():
    ...
    for sub in SUBJECTS:
        ...
        jpath = OUT_ROOT / f"09_PREEXP-SUB{sub}-FULL" / f"sub{sub}_full_windows.json"
        if jpath.exists():
