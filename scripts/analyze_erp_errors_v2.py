...
OUT_DIR = OUT_ROOT / "03_跨被试" / "09_预实验-事件相关-v2"
SUBJECTS = ["003", "004", "005", "006", "007", "008", "009", "010"]  # 8 人集 (001/002 摆位, 000 练习效应, 8-13 定)
WINDOW_SEC = 30.0          # 生理窗长（与 analyze_mmwave_full 一致）
MIN_EVENTS = 5             # 被试级分析最少事件数
RT_PREEMPT_MS = 150    # 预判阈值: RT<150ms 为预判性误按 (8-13 分型)
            ...
            for r in csv.DictReader(f):
                ...
                if r["is_no_go"] == "1" and r["response"] == "1":
                    try:
                        ...
                        rt = None
                    if rt is not None and rt < RT_PREEMPT_MS:
                        events.append({"type": "comm_preempt", "rel_s": rel_s})
                    else:
                        events.append({"type": "comm_true", "rel_s": rel_s})
...
def load_windows(subject: str) -> list[dict]:
    ...
    p = OUT_ROOT / "02_全程窗" / f"09_预实验-SUB{subject}-FULL-v6" / f"sub{subject}_full_windows.json"
    if not p.exists():
        return []
...
def classify_windows(windows: list[dict], events: list[dict]) -> tuple[dict, dict]:
    ...
    starts = sorted(idx)
    event_windows = {"comm_true": [], "comm_preempt": [], "omis": []}
    context = {"comm_true": {"prev": [], "post": []}, "omis": {"prev": [], "post": []}}
    for ev in events:
        t = ev["rel_s"]
        ...
        for s in starts:
            if s <= t - 5 and t + 5 < s + WINDOW_SEC:
                ...
                # 前窗/后窗（存在才记录; 预判性误按不做前后曲线, 仅窗口级对比）
                if ev["type"] in context:
                    j = starts.index(s)
                    if j > 0:
...
def main():
    ...
    for sub in SUBJECTS:
        ...
        n_comm, n_omis = len(set(ev_idx["comm_true"])), len(set(ev_idx["omis"]))
        n_preempt = len(set(ev_idx.get("comm_preempt", [])))
        print(f"sub-{sub}: 可信窗 {len(windows)}, commission {n_comm}, omission {n_omis}")
        sub_res = {"n_windows": len(windows), "n_comm": n_comm, "n_preempt": n_preempt, "n_omis": n_omis,
                   "compare": {}, "response": {}}
        for typ, n in (("comm_true", n_comm), ("omis", n_omis)):
            if n < MIN_EVENTS:
                continue
            for m in METRICS:
    ...
    for sub, r in result.items():
        if sub == "_aggregate":
            continue
        for m in METRICS:
            if f"comm_true_{m}" in r["compare"]:
                agg_d[m].append(r["compare"][f"comm_true_{m}"]["cohen_d"])
...
def _plot_compare(result: dict, out_dir: Path):
    ...
    for ax, m in zip(axes, METRICS):
        ...
        if ds:
            colors = ["#2e86c1" if abs(d) < 0.2 else ("#c0392b" if d > 0 else "#8e44ad") for d in ds]
            ax.barh(labels, ds, color=colors)
            ax.axvline(0, color="black", linewidth=0.8)
            import matplotlib.patches as mpatches
            ax.legend(handles=[
                mpatches.Patch(color="#2e86c1", label="小效应 (|d|<0.2)"),
                mpatches.Patch(color="#c0392b", label="错误窗更高 (d>0.2)"),
                mpatches.Patch(color="#8e44ad", label="错误窗更低 (d<-0.2)"),
