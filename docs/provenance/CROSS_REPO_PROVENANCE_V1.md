...
| central analysis | `main` | identity, folds, matched inference, report surface |
...
The current NIR diagnostic PR #2 is `chatgpt/multimodal-results-nir-diagnostic-20260826@0e756b275fd9cbbc7d7564531d3200425bf3be23`. It remains pre-recovery diagnostic evidence, not a final multimodal result. Its curated successors are listed in `results/engineering_reference/nir/NIR_FEATURE_CONTRACT_DIAGNOSTIC_V2.md` and `docs/archive/superseded/PR2_MULTIMODAL_STATUS_REFERENCE_20260826.md`.

The external producer refs are sufficient without local vendor mirror branches. The previously observed local `vendor/attention-amd-DirectML` and `vendor/attention-nvidia-cuda` tracking refs are `NON_CANONICAL_VENDOR_MIRROR / NOT_REQUIRED`; they are not recreated.
