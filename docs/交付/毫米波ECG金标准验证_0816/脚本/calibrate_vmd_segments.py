...
# ECG 伪迹剔除参数（2026-08-16 补：从 calibrate_ecg_mmwave.py 移植，
# 此前主线 ecg_hr_segment 仅靠 IBI 硬范围过滤，运动/电极伪迹未剔除）
ECG_AMP_MAD_N = 3.0             # R 峰幅度 MAD 离群倍数（默认 3.0，只抓极端离群，
                                #   留足呼吸调制对 R 波幅度的正常波动余量，可调 2.5~4.0）
IBI_JUMP_MAD_N = 3.0            # 相邻 IBI 相对突跳 MAD 离群倍数（默认 3.0，
                                #   剔除异位搏动/漏检造出的假间期，可调 2.5~4.0）


def _mad_threshold(values, n_sigma=3.0):
    """中位绝对偏差（MAD）自适应离群阈值。

    用中位而非均值、MAD 而非标准差，因伪迹会拉偏均值/方差，
    使阈值本身被污染；MAD 对离群鲁棒。
...
def _drop_ibi_artifacts(ibi):
    """IBI 生理一致性门控：剔除相邻间期突跳的伪迹拍。

    依据：正常窦性心律相邻 IBI 变化平缓（呼吸性窦性心律不齐 RSA 的波动
    通常 ±10~15%）；异位搏动、运动伪迹、峰漏检会造出相邻 IBI 突跳的假间期。
    ...
    _, high = _mad_threshold(rel, n_sigma=IBI_JUMP_MAD_N)
...
def ecg_hr_segment(acq_path, sr, t0_unix, t1_unix, align_offset, align_k):
    """ECG 段内心率（prominence R 峰检测 + 伪迹剔除）。

    align: unix = offset + k*idx。返回段内中位心率（bpm），
    无法检出足够 R 峰时返回 None。
    """
    import bioread
    ...
    peaks, props = find_peaks(seg, distance=int(0.3 * sr), prominence=0.25)
    if len(peaks) < 5:
        return None

    # 伪迹剔除①：R 峰幅度 MAD 门控。运动/电极松脱产生异常幅度尖峰，
    # 用 prominence 的 MAD 只抓极端离群，不误杀正常呼吸调制。
    proms = props['prominences']
    lo_amp, hi_amp = _mad_threshold(proms, n_sigma=ECG_AMP_MAD_N)
    peaks = peaks[(proms >= lo_amp) & (proms <= hi_amp)]
    if len(peaks) < 5:
        return None
