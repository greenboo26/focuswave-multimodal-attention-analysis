# Pre-probe behavior windows — method card

## Purpose

Define the scientific question and report role for Pre-probe behavior windows. This card records existing work only; it does not authorize a new exploratory run.
...
- **Question:** Probe 状态是否与其前方行为错误/RT 形成预定义效标关联
- **Producer:** scripts/run_beijing_preprobe_state_comparison_v1.py
- **Inputs / required columns:** probe_event_level_behavior.csv; target_label1, repeat_participant_id, pre10/20/30 behavior fields
- **Cohort and unit:** 70 sessions, 46 participants, 1400 probes
- **Model / validation:** Gaussian GEE exchangeable; outcome ~ state + probe_progress + block_num; BH over 18 contrasts
- **Execution role and boundary:** Beijing canonical validity supporting; 30s primary, 10/20 sensitivity
