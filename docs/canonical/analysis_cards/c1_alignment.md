# C1 alignment/beat/IBI/HRV validation — method card

## Purpose

Define the scientific question and report role for C1 alignment/beat/IBI/HRV validation. This card records existing work only; it does not authorize a new exploratory run.
...
- **Question:** C1 是否支持本周期停止 HRV 验证
- **Producer:** existing C1 alignment protocol repair producer; no AMD rerun
- **Inputs / required columns:** c1_alignment_protocol_repair_v1 aggregate audits
- **Cohort and unit:** paired RS6240 subset
- **Model / validation:** held-out lag/IBI alignment diagnostics
- **Execution role and boundary:** VALIDATION_STOPPED supporting; not external-storage failure
