# Final RGB increment — method card

## Purpose

Define the scientific question and report role for Final RGB increment. This card records existing work only; it does not authorize a new exploratory run.
...
- **Question:** RGB 是否在 C+B 之上增加信息
- **Producer:** future Sol-approved RGB runner; not executed here
- **Inputs / required columns:** local standardized RGB package after identity reconciliation
- **Cohort and unit:** same matched cohort/folds as NIR
- **Model / validation:** same scientific contract; no cohort-specific fold tuning
