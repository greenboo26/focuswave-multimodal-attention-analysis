# Final NIR increment — method card

## Purpose

Define the scientific question and report role for Final NIR increment. This card records existing work only; it does not authorize a new exploratory run.
...
- **Question:** NIR 是否在 C+B 之上增加信息
- **Producer:** future Sol-approved NIR runner; not executed here
- **Inputs / required columns:** local standardized NIR package after identity reconciliation
- **Cohort and unit:** complete multimodal common cohort
- **Model / validation:** L2 logistic; 30s primary, 10/60 sensitivity; paired participant bootstrap delta AUC
- **Execution role and boundary:** global-only inference
