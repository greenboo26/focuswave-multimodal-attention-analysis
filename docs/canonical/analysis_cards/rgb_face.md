# RGB Face benchmark/CUDA gate/formal runner — method card

## Purpose

Define the scientific question and report role for RGB Face benchmark/CUDA gate/formal runner. This card records existing work only; it does not authorize a new exploratory run.
...
- **Question:** RGB face backend 是否满足共享契约
- **Producer:** remote backend-specific runner; parity pending
- **Inputs / required columns:** RGB frame/face outputs plus model hash
- **Cohort and unit:** available videos
- **Model / validation:** engineering/parity tests
- **Execution role and boundary:** no final inference before parity gate
