# Beijing formal behavior longitudinal — method card

## Purpose

Define the scientific question and report role for Beijing formal behavior longitudinal. This card records existing work only; it does not authorize a new exploratory run.
...
- **Question:** 任务进度、行为错误和 Probe 状态是否形成行为效度链
- **Producer:** scripts/run_beijing_longitudinal_event_analysis_v1.py
- **Inputs / required columns:** beijing.../deterministic_join.csv and probe_event_level_behavior.csv; probe_progress, block_num, pre30_error_rate, RT
- **Cohort and unit:** 70 PASS_FORMAL sessions, 46 participants, 1400 probes
- **Model / validation:** participant-clustered GEE with progress/block adjustment
- **Execution role and boundary:** Beijing supporting; do not duplicate validity headline
