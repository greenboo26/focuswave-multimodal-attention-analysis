...
第一，`COMMON_SUBSET_BASELINE_V1` 可以作为当前严格共同样本上的**多模态最小基线（common-subset multimodal minimum benchmark）**，但暂不称为“multimodal upper-bound”。原因是当前融合模型并未形成性能上界：Primary 集合中 time/block structural baseline 的受试者工作特征曲线下面积（area under the receiver operating characteristic curve [AUC]）为 .680，而 behavior + radar + near-infrared（近红外成像；near-infrared [NIR]）融合为 .641；Sensitivity 集合分别为 .663 与 .621。该结果只说明当前低复杂度模型在严格共同样本上的起点，不代表多模态潜在上限。
