...
| O-006 | official ECG/RSP 8/16 workflow applicability per device | reference quality may differ by dataset | `ecg_reference_v1` is implemented and passes 60/60 AgeBalanced development-session full-session QC; `rsp_reference_v1` remains inventory/contract only. RS6240 inventory found ECG 11/RSP 10, two ID mismatches and missing derived linkage | PARTIAL |
...
| O-013 | historical 25 s result cannot be represented by frozen V1 schema | direct equivalence rows cannot be part of the fair-comparison ledger | retain 25 s as diagnostic; require separately approved contract version before any schema change | BLOCKED |
| O-014 | historical quality aggregation and harmonic-lock classifier/output are not recoverable from current artifacts | 1.6/3.4/10.1 and 4/1188/0 cannot be exactly rederived or compared | recover historical JSON/report or source implementation; otherwise retain MISSING_EVIDENCE | MISSING_EVIDENCE |
